...
**Architecture:** A Next.js 16 app behind a small custom Node server. The Node server routes by `Host`: `platform.alversjo.land` goes to Next.js, `<box-id>.boxes.alversjo.land` goes to the box proxy. The box proxy never touches the database; it asks a localhost-only internal Next route (guarded by a per-process secret) to resolve the session cookie into "forward to machine X with CloudCLI token Y", then streams HTTP and WebSocket traffic to the box over Fly's private network. Data lives in Postgres in prod and PGlite on dev boxes, through one Drizzle schema and one migrations folder.
...
1. Sign up a GitHub user `alversjo-contributor` with a mailbox you control.
...
for r in box platform dns; do gh api -X PUT repos/Alversjo-org/$r/collaborators/alversjo-contributor -f permission=push; done
```

3. Logged in as the bot, create a fine-grained personal access token: resource owner `Alversjo-org`, repositories `box`, `platform`, `dns`, permissions Contents read/write, Pull requests read/write, Metadata read. No expiry shorter than a year. This is `BOX_GH_TOKEN_CONTRIBUTOR`.
...
- `CLOUDFLARE_API_TOKEN`: the account-owned token already used by the `dns` repo.
- `RESEND_API_KEY`: the Resend key for `notifications.theborderland.se`. Rotate it after first deploy.
...
RESEND_API_KEY=
EMAIL_FROM=Alversjö <no-reply@notifications.theborderland.se>
# Fly
FLY_API_TOKEN=
...
export async function sendOtpEmail({ to, otp, type }: OtpMail): Promise<void> {
  ...
  const from = process.env.EMAIL_FROM ?? 'Alversjö <no-reply@notifications.theborderland.se>';
...
[env]
  PORT = "8080"
  PLATFORM_URL = "https://platform.alversjo.land"
...
curl -sI https://platform.alversjo.land/login | head -1
```

Expected: release command logs `migrations applied`; one machine `started`; `HTTP/2 200`. Log in at `https://platform.alversjo.land/login` with an admin email; the code arrives from `notifications.theborderland.se`; `/boxes` shows the empty list with a "New box" button.

- [ ] **Step 4: Rotate the chat-shared keys**

Create a new Resend key and a new Cloudflare token, `fly secrets set` them on `alversjo-platform`, update the `dns` repo secret, and revoke the old ones.
